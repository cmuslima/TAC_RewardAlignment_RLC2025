...
class Expected_Sarsa:
    ...
    def get_expected_q_value(self, state, epsilon):
        """ Compute the expected Q-value across all actions for a given state. """
        if isinstance(self.Q_table, np.ndarray):
            q_values = self.Q_table[state, :]
        else:
            action_vals_dict = self.Q_table[self.env.hash_lookup(state)]
            q_values = np.array([action_vals_dict[a] for a in range(self.env.action_space.n)])
        ...
        best_action = np.argmax(q_values)
         ...
         # Compute probabilities under ε-greedy policy
        action_probs = self.get_action_probs(num_actions, epsilon, best_action)
        ...
        return expected_q
    def get_action_probs(self, num_actions, epsilon, best_action):
        # Calculate probability for non-best actions
        non_best_prob = epsilon / num_actions
        

        # Initialize probabilities
        action_probs = np.full(num_actions, non_best_prob)
        assert np.isclose(np.sum(action_probs), epsilon, rtol=1e-7)

        # Set best action probability to ensure sum is exactly 1
        action_probs[best_action] = 1.0 - (epsilon * (num_actions - 1) / num_actions)
        assert np.isclose(np.sum(action_probs), 1.0, rtol=1e-7)
        return action_probs
    ...
    def learn_1_episode(self, alpha_lr, gamma, epsilon, new_water_food_loc= False, record=True):
        ...
        while not done:
            ...
            new_state, reward, done, info = self.env_interaction(state=state, action=action)
            # update the Q table
            expected_q = self.get_expected_q_value(new_state, epsilon)

            update_value = alpha_lr * (reward + gamma * expected_q)
...
def create_expected_sarsa_agent(env, hyper_params, user_reward_fn, dir, new_water_food_loc=False, train=True):
    """
    ...
    alg = Expected_Sarsa(env, num_episodes=num_episodes, reward_fn=user_reward_fn, epsilon=epsilon, new_water_food_loc=new_water_food_loc, dir=dir)
    if train:
        if num_episodes == 1:
