...
class DDQN_Agent:
    ...
    def test_performance(self, env, agent, epsilon=0, num_tests=1000, metric='reward', reward_fn=None, new_water_food_loc=False):
        """
        ...
        # if metric == 'reward':
        #     if self.best_mean_performance is None or np.mean(episode_rewards) > self.best_mean_performance:
        #         self.best_mean_performance = np.mean(episode_rewards)
...
def create_ddqn_agent(env,
                      ...
                      plot_results=False,
                      device=torch.device('cpu'), new_water_food_loc=False):
    """
    ...
    final_episode_rf, final_episode_re, final_episode_fit = agent.test_performance(env, agent, reward_fn = user_reward_fn, new_water_food_loc=new_water_food_loc)
    ...
    if plot_results:
        fig.end_interactive()
    return agent, epsilon, episode_rewards, episode_fitness, episode_returns, trajs, final_episode_fit
