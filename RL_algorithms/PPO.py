...
def test_performance(env, agent, num_tests=1000, reward_fn=None, new_water_food_loc=False):
    """
    ...
    for _ in tqdm(range(num_tests)):
        obs = env.reset(new_water_food_loc=new_water_food_loc)
        done = False
        episode_reward = 0
        episode_fitness = 0
        episode_return = 0
        ...
        while not done:
            ...
            action, action_log_probs = agent.select_action(state=state_as_arr)
            next_obs, reward, done, info = env.step(action.item())

            if reward_fn is not None:
                reward = reward_fn(state=obs, action=action.item(), new_state=next_obs)
                episode_reward += reward
                episode_return += (0.99 ** steps) * reward
            if not obs["hungry"]:
                episode_fitness += 1
        ...
        env.close()
    return episode_rewards, episode_returns, fitnesses
def train_ppo_agent(env,
    ...
    for episode_idx in tqdm(range(num_episodes-1)):
        state = env.reset(new_water_food_loc=new_water_food_loc)
        episode_return = 0
        fitness = 0
        total_reward = 0
        steps = 0
        ...
        while not done:
            ...
            ppo_agent.rollout_buffer.add_experience(action=action,
                                                    ...
                                                    entropy=None)
            traj.append((state,int(action.item())))
            ...
            total_reward += reward
            episode_return += (gamma ** steps) * reward
            ...
            # update the plot
            if plot_results and total_steps != 0 and total_steps % 10000 == 0:
            ...
            if done:
                ...
                break
    final_episode_rf, final_episode_re, final_episode_fit = test_performance(env, ppo_agent, reward_fn=user_reward_fn)
    episode_rewards.append(np.mean(final_episode_rf))
    episode_returns.append(np.mean(final_episode_re))
    episode_fitness.append(np.mean(final_episode_fit))
  
    if plot_results:
        fig.end_interactive()
    return ppo_agent, episode_rewards, episode_fitness, episode_returns, trajs, final_episode_fit
...
def create_ppo_agent(env, hyper_params, user_reward_fn, plot_state_visit_distribution=False, plot_results=False, new_water_food_loc=False):
    """
    ...
    ppo_agent, undiscounted_return_over_time, fitness_over_time, discounted_return_over_time, traj, final_fitness = train_ppo_agent(env=env,
                                                           hyper_params=hyper_params,
                                                           user_reward_fn=user_reward_fn,
                                                           plot_results=plot_results,
                                                           plot_state_visit_distribution=plot_state_visit_distribution)

    return ppo_agent, undiscounted_return_over_time, fitness_over_time, discounted_return_over_time, traj, final_fitness
