...
class Sarsa:
    ...
    def learn_1_episode(self, alpha_lr, gamma, epsilon, new_water_food_loc= False, record=True):
        ...
        while not done:
            ...
            # update the Q table
            sampled_next_action = self.e_greedy_action_selection(state=new_state, epsilon=epsilon)
            new_state_q_val = self.q_lookup(state=new_state, action=sampled_next_action)
...
def create_sarsa_agent(env, hyper_params, user_reward_fn, dir, new_water_food_loc=False, train=True):
    """
    Create a Sarsa agent and return its learning performance
    ...
    alg = Sarsa(env, num_episodes=num_episodes, reward_fn=user_reward_fn, epsilon=epsilon, new_water_food_loc=new_water_food_loc, dir=dir)
    if train:
        if num_episodes == 1:
