...
###########
        ...
        if "agent_cell" not in self.render_tracking:
            ...
            # Send the rectangle to the background (so agent image stays on top)
            self.render_tracking["canvas"].lower(self.render_tracking["agent_cell"])
        else:
        ...
        #this was also to fix the agent not showing up
        self.render_tracking["agent_photoImg"] = ImageTk.PhotoImage(self.AGENT_IMG)
        self.render_tracking["agent_obj"] = self.render_tracking["canvas"].create_image(
            self.state["position"][0] * self.CELL_WIDTH +
            int(self.CELL_WIDTH / 2) - int(self.AGENT_IMG.width / 2) + self.OFFSET,
        ...
        if action == 4:
            ...
            if "action_text" not in self.render_tracking:
                ...
                self.render_tracking["canvas"].coords(
                    ...
                    (cell_x1 + cell_x2) / 2,
                    (cell_y1 + cell_y2) / 2 - 50  # Adjust position to be above the agent
                )
            ...
            if "action_text" not in self.render_tracking:
                ...
                self.render_tracking["canvas"].itemconfigure(
                    self.render_tracking["action_text"], text="drink", fill="red"
                )

        # ✅ Improved the else condition to prevent flickering
        elif action in {0, 1, 2, 3}:
            # Check if "action_text" exists before deleting
...
###########
        ...
        self.render_tracking["canvas"].itemconfigure(
            self.render_tracking["step_text"],
            text="Step: " + str(self.step_ct),
            font=("Times", 35)  # Increase the font size
        )
        ...
        if state_counter is not None:
            bottom_padding = self.CANVAS_HEIGHT - self.OFFSET + 70  # Adjust padding
            box_size = 20  # Size of the color box
            text_padding = 40  # More space between the box and text
            row_spacing = 35  # Increase spacing between rows

            # Define colors for each state
            state_colors = {
                "not_hungry_not_thirsty": "green",
                "not_hungry_thirsty": "green",
                "hungry_thirsty": "white",
                "hungry_not_thirsty": "blue"
            }
            name_mapping = {
                "not_hungry_not_thirsty": "Not Hungry and Not Thirsty",
                "not_hungry_thirsty": "Not Hungry and Thirsty",
                "hungry_thirsty": "Hungry and Thirsty",
                "hungry_not_thirsty": "Hungry and Not Thirsty" 
            }

            # Left side (Not Hungry states)
            left_states = ["not_hungry_not_thirsty", "not_hungry_thirsty"]
            for i, state in enumerate(left_states):
                y_position = bottom_padding - (i * row_spacing)  # Adjust spacing

                # Create or update the color box
                if f"{state}_box" not in self.render_tracking:
                    self.render_tracking[f"{state}_box"] = self.render_tracking["canvas"].create_rectangle(
                        10, y_position - box_size // 2, 10 + box_size, y_position + box_size // 2,
                        fill=state_colors[state], outline="black"
                    )

                # Create or update the text
                if f"{state}_text" not in self.render_tracking:
                    self.render_tracking[f"{state}_text"] = self.render_tracking["canvas"].create_text(
                        10 + text_padding, y_position,
                        text = f"{name_mapping[state]}: {state_counter[state]}",
                        font=("Times", 22, "bold"),  # Slightly smaller font
                        anchor="w"
                    )
                else:
                    # Update existing text instead of creating new one
                    self.render_tracking["canvas"].itemconfigure(
                        self.render_tracking[f"{state}_text"],
                        text = f"{name_mapping[state]}: {state_counter[state]}"
                    )

            # Right side (Hungry states)
            right_states = ["hungry_thirsty", "hungry_not_thirsty"]
            for i, state in enumerate(right_states):
                ...
                if f"{state}_box" not in self.render_tracking:
                    self.render_tracking[f"{state}_box"] = self.render_tracking["canvas"].create_rectangle(
                        self.CANVAS_WIDTH - 10 - box_size, y_position - box_size // 2,
                        self.CANVAS_WIDTH - 10, y_position + box_size // 2,
                        fill=state_colors[state], outline="black"
                    )
                ...
                if f"{state}_text" not in self.render_tracking:
                    self.render_tracking[f"{state}_text"] = self.render_tracking["canvas"].create_text(
                        self.CANVAS_WIDTH - 10 - text_padding, y_position,
                        text = f"{name_mapping[state]}: {state_counter[state]}",
                        font=("Times", 22, "bold"),
                        anchor="e"
                    )
        ...
        if episode is not None:
            self.render_tracking["canvas"].itemconfigure(
                self.render_tracking["episode_text"],
                text="Trajectory: " + str(episode),
                font=("Times", 35)  # Bigger and bold
            )
    ...
    def update_state(self, state):
        ...
        self.last_position = self.state["position"]
        self.state = state
    def playback(self, trajectory, episode_metadata, trajectory_id, render=True):
        """
        ...
        while len(trajectory) > 0 and self.window_closed == False:
            ...
            if render:
                self.render_with_episode(state_counter=state_counter,episode=trajectory_id, action=action, timestep=timestep)
            closed_rendering_early = self.window_closed
            time.sleep(0.20)
        ...
        #print('total score', total_score)
        self.close()
        #print('closed_rendering_early', closed_rendering_early)
        #self.save_gif(gif_filename=f"demo_gif.gif")
        return closed_rendering_early, total_score
    ...
    def close(self):
        ...
        if self.window_closed == False:
            close = messagebox.askyesno("Exit", "Render is over. Press Yes to Exit window")
            if close:
                if self.render_tracking:
                    self.canvas_root.destroy()
        self.render_tracking = {}
