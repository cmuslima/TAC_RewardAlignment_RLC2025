...
class interface_backend:
    def __init__(self, random_seed=28):
        ...
        self.real_user = True
        self.use_loaded_preference_data = False
        ...
        self.study_step = 0
        self.num_preferences_complete = 0
        self.num_preferences_total = int(self.num_traj*(self.num_traj-1)/2)
        self.render_trajectories_in_visualization = False
        ...
        #reward_funcs_considered = [(29, 28), (12, 21), (29, 24), (17, 10), (13, 14), (18, 37), (29, 32), (34, 37), (15, 17)]
        
        
        reward_funcs_considered = [
                                     (12, 21),
                                    (13, 18),
                                    (13, 14),
                                    (18, 37),
...
# (24, 29), (22, 21), (28, 29), #maybe removed this
       
        self.ground_truth, self.selected_reward_functions = user_study_utils.get_specific_reward_functions(reward_funcs_considered)
        ...
        self.data_to_save = {'name': None, 'user_id': None, 'random_seed': self.random_seed, 'env_quiz_responses': None, 'practice_rankings': None, 'user_rankings': None,\
             'preference_comparison_time': None, 'preference_survey': None, 'total_study_time': None, 'comparison_survey_time': None, 'comparison_survey_responses': None, \
            'close_window_early_rewatching': [], 'close_window_early': []}
    ...
    def replay_and_pair_wise_rank_trajectories(self, practice_mode=True):
        ...
        def run_comparison(idx1, idx2, practice_mode=False):
            ...
            if self.real_user:
                ...
                fakechoice = self.replay_trajectory(idx1, idx2)
    ...
    def replay_trajectory(self, idx1, idx2):
        """
    ...
    def draw_rankings(self, rankings_1, rankings_2, reward_function_name):
        ...
        def display_mismatch_details(drop_items, increase_items, mismatched_trajectories):
            ...
            if mismatched_trajectories:
                details = [
                    "Bolded lines are trajectories with the greatest difference in rank.\n"
                ...
                for trajectory in increase_items:
                    ...
                    if rank1 is not None and rank2 is not None:
                        ...
                        if abs(increase_diff) > 0:
                            details.append(f"   - {trajectory}: You ranked it {increase_diff} slots higher.")
                ...
                for trajectory in drop_items:
                    ...
                    if rank1 is not None and rank2 is not None:
                        ...
                        if abs(drop_diff) > 0:
                            details.append(f"   - {trajectory}: You ranked it {abs(drop_diff)} slots lower.")
    ...
    def visualize_rankings_or_replay_no_alignment(self, all_reward_function_names, all_reward_function_rankings):
        ...
        while True:
            ...
            print("1: Visualize how your preferences over trajectories differ from the preferences induced by the reward functions.")
            ...
            if choice == 'stop':  # Quit option
                ...
                #         if len(mismatched_trajectories) > 0:
                #             print('This includes:')
                ...
                try:
                    ...
                    trajectory_choice = input("Enter the letter of the trajectory to replay:")
                    trajectory_choice = trajectory_choice.lower()
                    ...
                    #
                    #need to save here for whether the traj was exited or not. 
                    close_early , _ = self.env.playback(list(self.trajectories[trajectory_idx][:]), self.metadata[trajectory_idx], trajectory_choice)  # Pass a copy of the trajectory
    ...
    def visualize_rankings_or_replay(self, all_reward_function_names, all_reward_function_alignment_scores, all_reward_function_rankings):
        ...
        while True:
            # Ask the user for their choice
            print("\nTo help you decide which reward to select, you can")
            print("1: Load the Alignment Scores")
            print("2: Visualize how your preferences over trajectories differ from the preferences induced by the reward functions.")
            ...
            if choice == 'stop':  # Quit option
                ...
                for idx, reward_function in enumerate(all_reward_function_names):
                    print(f'\n Reward Function {idx+1}:')
                    for key in reward_function.keys():
                        print(f'{key} = {reward_function[key]}')
                    print(colored(f"With alignment score = {np.round(all_reward_function_alignment_scores[idx], 4)}", "yellow", attrs=["bold"]))
    ...
    def print_condition_intro(self, condition_name, iteration, num_iterations):
        print('\n')
        print(colored(f"Reward Function Comparison # {iteration+1} out of 3 for Condition:\n {condition_name}", "black", attrs=["bold"]))
        ...
        if iteration == 0:
            ...
            if condition_name in ['Visual + Alignment Feedback', 'Alignment Feedback']:
                print('2. An alignment score, which is a score ranging in the interval [-1,1].\n')
                print('This alignment score is based on the preferences you supplied in Task 1. ')
                print('This score will tell you how similar your preferences are with the preferences induced by the reward functions.\n')
                self.print_alignment_score_explanation()
            if condition_name == 'Visual + Alignment Feedback':
                print('3. Visualizations that highlight how your preferences over trajectories differ from the preferences induced by the reward functions.\n')
                print('4. The ability to rewatch any trajectories you like.\n')
            if condition_name == 'Visual Feedback':
                print('2. Visualizations that highlight how your preferences over trajectories differ from the preferences induced by the reward functions.\n')
                print('3. The ability to rewatch any trajectories you like.\n')        
            input('Press Enter to continue.')
            print('\n')

    def print_alignment_score_explanation(self):
        print(f"An alignment score close to {colored('1', 'green', attrs=['bold'])} indicates that the reward function induces {colored('similar preferences as you!', 'green', attrs=['bold'])}\n")
        print(f"An alignment score close to {colored('-1', 'red', attrs=['bold'])} indicates that the reward function induces {colored('opposite preferences as you!', 'red', attrs=['bold'])}\n")
    ...
    def print_condition_summary(self):
        ...
        print("1. Reward Functions\n")
        print("2. An alignment score that indicates how similar your preferences are to those induced by the reward functions.\n")
    ...
    def visualization_reward_func_condition(self, reward_function_names_selected, ranks, iteration):
        ...
        #self.data_to_save[self.current_function_name]['clicked_trajectories'].append(clicked_trajectories)
        print('=' * 90)
    def reward_func_only_condition(self, reward_function_names_selected, iteration):
    ...
    def _prepare_arguments(
        ...
        if self.current_func == self.visualization_reward_func_condition:
            ...
            return reward_function_names_selected, iteration
        else:
    ...
    def condition_survey(self):
        ...
        start_time = time.time()
        root = tk.Tk()
        responses = user_study_utils.run_survey(root)
        root.quit() 
    ...
    def get_trajectories(self):
        ...
        file = f'{os.getcwd()}/User_Study_Data/Trajectories/trajectory_comparsions_start_state_10_0_0_seed0.pkl'
        traj_ids =utils.get_pickle_data(file)


        trajectories, _ = utils.get_trajectories(self.ground_truth, 'Q_Learn', self.env_seed+1, dir=trajectory_data_dir)
        self.trajectories = trajectories[self.env_seed][traj_ids]
        ...
        #self.reward_function_rankings = OrderedDict(self.reward_function_rankings)
    ...
    def studypart1(self):
        ...
        if self.use_loaded_preference_data:
            ...
            finish_time = time.time()
            total_time = finish_time-start_time
            self.data_to_save['preference_comparison_time'] = total_time
